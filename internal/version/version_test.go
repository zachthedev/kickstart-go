...
//
// Fully isolated from the user's git config (no global/system config read,
// signing disabled) so the test cannot trip a GPG agent or inherit
...
func TestFromGit_InRepo(t *testing.T) {
	...
	git := func(args ...string) {
		...
		cmd := exec.Command("git", append(flags, args...)...) //nolint:gosec
