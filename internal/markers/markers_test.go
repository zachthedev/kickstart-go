...
// A directory holding a .git entry is another work tree, such as a Claude
// Code worktree under the gitignored .claude/worktrees. Scanning it counts
// every marker twice and turns the inventory stale in the main checkout.
func TestScan_SkipsNestedWorkTrees(t *testing.T) {
	tests := []struct {
		name  string
		files map[string]string
	}{
		{"linked worktree, .git file", map[string]string{".claude/worktrees/wt/.git": "gitdir: elsewhere\n"}},
		{"nested repository, .git directory", map[string]string{".claude/worktrees/wt/.git/HEAD": "ref: refs/heads/main\n"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			files := map[string]string{
				"keep.go":                         TODOMarker + "\n",
				".claude/worktrees/wt/a.go":       TODOMarker + "\n",
				".claude/worktrees/wt/MARKERS.md": TODOMarker + "\n",
			}
			for rel, body := range tt.files {
				files[rel] = body
			}
			got, err := Scan(writeTree(t, files), "MARKERS.md")
			require.NoError(t, err)
			require.Len(t, got, 1, "only the main checkout's file counts")
