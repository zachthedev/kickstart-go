...
// marker, sorted by path. Binary files and the paths in skip are ignored;
// pass the generated inventory's own path in skip so it does not count
// itself.
func Scan(root string, skip ...string) ([]File, error) {
	...
	err = filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			if slashed != "." && isSkippedDir(slashed) {
				return fs.SkipDir
			}
			return nil
		}
