...
// there is fine, since the log has to start somewhere. A lookup through a
// regular file answers ENOTDIR on Unix and a not-found error on Windows.
// Both mean nothing is at path. The MkdirAll in [New] then reports the
// failure on every OS.
