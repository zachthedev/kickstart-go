...
#
# `task test` is the only caller, and it owns TEST_TARGET. This script
# refuses an unset value rather than substituting one, so the suite a task
# runs and the suite a bare invocation runs cannot differ.
#
...
CGO_ENABLED=1 go test -race -count=1 ${TAGS:+-tags "$TAGS"} $TEST_TARGET
