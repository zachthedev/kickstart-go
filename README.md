...
`0.x` promises no compatibility between releases.
...
deliberate divergence from template content carries a `NOTE` in the same scope. `git grep -n "(kickstart)"`
finds both.
...
Not needed if you enforce no coverage threshold. Delete `.testcoverage.yml`, drop the `coverage` task
from `Taskfile.yml` and from `check`, remove `github.com/vladopajic/go-test-coverage/v2` from the `tool`
block in `go.mod`, and run `go mod tidy`.

### Test pairing (`testpair`)

Not needed if you enforce no 1:1 pairing. Delete `.allow.testpair`, drop the `testpair` task and its
entry in `check`, remove `zach.tools/go/devtools/cmd/testpair` from the `tool` block, and run `go mod tidy`.
...
`deadcode` task and its entry in `check`, remove `zach.tools/go/devtools/cmd/deadcode` from the `tool`
block, and run `go mod tidy`.
...
Drop the `generate` task from `Taskfile.yml` and from `check`, and the `generate` job from `lefthook.yml`.
...
`.golangci.yml` with the reason beside it, not a disabled linter.
