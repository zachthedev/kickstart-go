// TODO(kickstart): set the module path to your project's import path, then
// update every remaining reference. This lists them all, so nothing has to be
// remembered:
//
//	git grep -l zach.tools/go/kickstart
module zach.tools/go/kickstart
...
require (
	...
	// Pinned to a v2 branch commit for unstable/edit, which internal/migrate's
	// TOMLRegistry needs and which no tagged release carries yet. Move to a
	// release tag once one ships with the package.
